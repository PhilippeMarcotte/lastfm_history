...
def get_songs_from(artist, album):
  sql = f"""
         SELECT album_order, MAX(date) as date, name, artist, album, (SELECT COUNT(*) FROM scrobbles as S WHERE S.song = name AND S.album = album AND S.artist = artist) as count
         FROM songs
